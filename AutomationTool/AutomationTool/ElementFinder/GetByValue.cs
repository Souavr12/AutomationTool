...
{
    internal class GetByValue : Interfaces.IElementFinder, Interfaces.ICollectionFinder
    {
        public BrowserResult<IHTMLElement> FindElement(IHTMLElement ThisElement, FieldProperty fieldAttribute, string Value, StringMatcher MatchStringBy, string AttributName = "")
        {
            ...
            {
                string theValue = ThisElement.getAttribute("value").ToString().Trim().ToLower();
                bool flag = false;
        ...
        public BrowserResult<List<IHTMLElement>> FindCollection(IHTMLElementCollection ThisCollection, FieldProperty fieldAttribute, string Value, StringMatcher MatchStringBy, string AttributName = "")
        {
            ...
            if (ThisCollection != null && ThisCollection.length > 0)
            {
                FinalCollection = ThisCollection.Cast<IHTMLElement>()
                                                .Where(elem => elem.getAttribute("value") != null)
                                                .Select(elem => elem).ToList();
                bool flag = false;
                switch (MatchStringBy)
                {
                    case StringMatcher.Exact:
                        if (FinalCollection.Any(elem => elem.getAttribute("value").ToString().Trim().ToLower() == Value.ToLower()))
                        {
                            FinalCollection = FinalCollection.FindAll(elem => elem.getAttribute("value").ToString().Trim().ToLower() == Value.ToLower());
                            flag = true;
                        }
                        break;
                    case StringMatcher.Start:
                        if (FinalCollection.Any(elem => elem.getAttribute("value").ToString().Trim().ToLower().StartsWith(Value.ToLower())))
                        {
                            FinalCollection = FinalCollection.FindAll(elem => elem.getAttribute("value").ToString().Trim().ToLower().StartsWith(Value.ToLower()));
                            flag = true;
                        }
                        break;
                    case StringMatcher.End:
                        if (FinalCollection.Any(elem => elem.getAttribute("value").ToString().Trim().ToLower().EndsWith(Value.ToLower())))
                        {
                            FinalCollection = FinalCollection.FindAll(elem => elem.getAttribute("value").ToString().Trim().ToLower().EndsWith(Value.ToLower()));
                            flag = true;
                        }
                        break;
                    default:
