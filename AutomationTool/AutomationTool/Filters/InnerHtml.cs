...
{
    internal class InnerHtml : IFilter
    {
        public BrowserResult<IHTMLElementCollection> FilterBy(IHTMLElementCollection thisCollection, StringMatcher matchStringBy, string value)
        {
            ...
            switch (matchStringBy)
            {
                case StringMatcher.Exact:
                    valueCollection = (thisCollection.Cast<IHTMLElement>().Where(elem => elem.innerHTML != null)
                                              .Where(elem => elem.innerHTML.Trim().ToLower() == value)) as IHTMLElementCollection;
                    break;
                case StringMatcher.Start:
                    valueCollection = (thisCollection.Cast<IHTMLElement>().Where(elem => elem.innerHTML != null)
                                              .Where(elem => elem.innerHTML.Trim().ToLower().StartsWith(value))) as
                                      IHTMLElementCollection;
                    break;
                case StringMatcher.End:
                    valueCollection = (thisCollection.Cast<IHTMLElement>().Where(elem => elem.innerHTML != null)
                                              .Where(elem => elem.innerHTML.Trim().ToLower().EndsWith(value))) as IHTMLElementCollection;
                    break;
                default:
